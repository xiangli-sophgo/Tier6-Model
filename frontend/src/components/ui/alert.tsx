  ...
  "relative w-full rounded-lg border px-4 py-3 text-sm [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-text-primary [&>svg~*]:pl-7",
  {
    variants: {
      variant: {
        ...
        destructive:
          "border-error/50 text-error bg-red-50 [&>svg]:text-error",
        success:
          "border-success/50 text-success bg-green-50 [&>svg]:text-success",
        warning:
          "border-warning/50 text-warning bg-yellow-50 [&>svg]:text-warning",
      },
    },
    defaultVariants: {
      variant: "default",
