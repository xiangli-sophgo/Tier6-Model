...
@op_registry.register("rmsnorm")
class RMSNormOp(OpBase):
    """RMSNorm operator."""
    ...
    def get_inputs(self) -> list[TensorDesc]:
        return [
            TensorDesc(name="input", shape=[self._b, self._s, self._d], dtype=self._dtype),
            TensorDesc(name="weight", shape=[self._d], dtype=self._dtype, is_weight=True),
        ]
