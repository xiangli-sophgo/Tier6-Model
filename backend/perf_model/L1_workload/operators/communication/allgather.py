"""AllGather operator implementation."""
...
@op_registry.register("allgather")
class AllGatherOp(OpBase):
    ...
    def get_outputs(self) -> list[TensorDesc]:
        elements = bytes_to_elements(self._bytes, self._dtype.bytes)
        return [TensorDesc(name="gathered", shape=[elements], dtype=self._dtype)]
