"""ReduceScatter operator implementation."""
...
from perf_model.L1_workload.operators.utils import bytes_to_elements, get_first_int
from perf_model.L1_workload.tensor import TensorDesc


@op_registry.register("reducescatter")
class ReduceScatterOp(OpBase):
    ...
    def get_outputs(self) -> list[TensorDesc]:
        elements = bytes_to_elements(self._bytes, self._dtype.bytes)
        return [TensorDesc(name="scattered", shape=[elements], dtype=self._dtype)]
