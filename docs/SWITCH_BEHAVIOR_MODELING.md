...
交换机的转发模式决定了帧从入口到出口的处理方式，直接影响延迟、可靠性和吞吐量。

#### Store-and-Forward 模式（存储转发）

**基本原理**：交换机完整接收整个帧后，进行 CRC 校验，确认无误后再转发。
...
延迟 = t1 - t0 (最后一个 bit 进入 → 第一个 bit 离开)
```
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                    Store-and-Forward 处理流水线                          │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐          │
│  │ 1. 接收   │───▶│ 2. 缓存   │───▶│ 3. CRC   │───▶│ 4. 决策   │          │
│  │   帧     │    │   完整帧  │    │   校验   │    │   转发   │          │
│  └──────────┘    └──────────┘    └────┬─────┘    └────┬─────┘          │
│                                       │               │                 │
│                                       ▼               ▼                 │
...
FCS (Frame Check Sequence) 是以太网帧尾部的 4 字节字段，包含 CRC-32 校验值。

```
以太网帧结构:
┌──────────┬──────────┬──────────┬─────────────┬────────────┬─────────┐
│ 前导码    │ 目标MAC  │  源MAC   │ 类型/长度   │   数据      │   FCS   │
│ 8 字节   │  6 字节  │  6 字节  │   2 字节    │ 46-1500字节 │  4 字节 │
└──────────┴──────────┴──────────┴─────────────┴────────────┴─────────┘
                                                              │
                                                   CRC-32 校验值
```
...
**CRC 校验过程**：
1. 交换机接收完整帧并存入缓冲区
2. 对帧数据（不含 FCS）进行 CRC-32 多项式计算
3. 将计算结果与帧中的 FCS 字段比较
4. 匹配则转发，不匹配则丢弃
...
**延迟公式**：
```
T_store_forward = T_serialization_in + T_processing + T_queueing
                = (frame_size × 8 / BW_in) + T_crc + T_lookup + T_queue

示例 (1518字节帧, 100GbE):
  T_serialization = 1518 × 8 / 100Gbps = 121.44 ns
  T_processing    ≈ 300-500 ns
  总延迟          ≈ 400-700 ns (不含排队)
```
...
| 方面 | 说明 |
|------|------|
| ✅ 完整错误检测 | 丢弃所有 CRC 错误帧，防止错误传播 |
| ✅ 支持 QoS | 可检查完整帧内容进行流量分类 |
| ✅ 支持速率转换 | 可在不同速率端口间转发 |
| ❌ 延迟较高 | 必须等待完整帧，延迟与帧大小成正比 |
| ❌ 缓冲区需求大 | 需要存储完整帧 |
...
**基本原理**：交换机只读取帧头部的目标 MAC 地址（前 6 字节），立即开始转发，无需等待完整帧。
...
**CAM 表（Content Addressable Memory）查找**：

CAM 表是交换机用于 MAC 地址查找的高速硬件表，也称为 MAC 地址表或转发表。
...
│     MAC 地址         │   端口   │  VLAN  │   老化时间   │
├─────────────────────┼──────────┼────────┼─────────────┤
│ AA:BB:CC:DD:EE:01   │  Port 1  │  VLAN1 │   280 sec   │
│ AA:BB:CC:DD:EE:02   │  Port 3  │  VLAN1 │   150 sec   │
│ AA:BB:CC:DD:EE:03   │  Port 7  │  VLAN2 │   45 sec    │
│        ...          │   ...    │  ...   │     ...     │
└─────────────────────┴──────────┴────────┴─────────────┘

查找时间: O(1) - 硬件并行比较，通常 < 100ns
默认老化时间: 300 秒
```
...
**CRC Stomping（CRC 标记）机制**：

Cut-Through 模式下，交换机在转发时还未收到 FCS，无法校验 CRC。但交换机会在帧完全通过后检测 CRC：
...
错误帧 (CRC Stomping):
  入口 ──▶ [数据流式转发] ──▶ 出口
           最后检查 CRC ──▶ CRC 错误!
                               │
                               ▼
           在 FCS 位置写入特殊标记 (Stomped CRC)
           下游设备识别后丢弃该帧
```

**Stomped CRC 值**：通常是一个已知的非法 CRC 值，下游的 Store-and-Forward 交换机或终端设备会识别并丢弃。

**延迟公式**：
```
T_cut_through = T_header + T_lookup
              = (14 × 8 / BW) + T_cam_lookup
              ≈ 1.12ns (100GbE) + 100-300ns
              ≈ 100-400 ns (与帧大小无关)
```
...
| 方面 | 说明 |
|------|------|
| ✅ 超低延迟 | 延迟与帧大小无关，适合延迟敏感应用 |
| ✅ 高吞吐量 | 流水线处理，无需等待完整帧 |
| ❌ 无错误过滤 | 错误帧会被转发（依赖 CRC Stomping） |
| ❌ 速率限制 | 入口/出口端口速率必须匹配 |
| ❌ 无 QoS 支持 | 无法检查完整帧内容 |

---

#### Fragment-Free 模式（碎片过滤/无残帧转发）

也称为 **Modified Cut-Through** 或 **Runtless Switching**。

**基本原理**：等待接收前 64 字节后再开始转发，可过滤因碰撞产生的残帧。
...
延迟 = t1 - t0 = 接收64字节的时间 + 处理时间
```

**为什么是 64 字节？—— Slot Time 与碰撞窗口详解**

这与以太网 CSMA/CD 机制密切相关：
...
物理约束:
  ...
  - 最多 4 个中继器
  - 最大网络直径: 2500m
  - 信号传播速度: ~2×10⁸ m/s (光纤/铜缆)

往返时间 (RTT):
  RTT = 2 × 2500m / (2×10⁸ m/s) = 25 μs

加上中继器延迟和安全余量:
  实际 RTT ≈ 46.4 μs

在此期间传输的比特数:
  bits = 10 Mbps × 46.4 μs = 464 bits

加上 Jam 信号 (48 bits):
  总计 = 464 + 48 = 512 bits = 64 bytes

这就是 Slot Time (时隙时间):
  10Mbps:  512 bits / 10 Mbps  = 51.2 μs
  100Mbps: 512 bits / 100 Mbps = 5.12 μs
  1Gbps:   512 bits / 1 Gbps   = 512 ns
```

**不同速率的 Slot Time 与网络直径**：

| 速率 | Slot Time | 最大网络直径 | 最小帧大小 |
|------|-----------|-------------|-----------|
| 10 Mbps | 51.2 μs | 2500m | 64 bytes |
| 100 Mbps | 5.12 μs | 250m | 64 bytes |
| 1 Gbps | 512 ns | 25m (理论) | 64 bytes (实际用载波扩展) |
| 10 Gbps+ | - | - | 全双工，无 CSMA/CD |

**Runt 帧（残帧）的产生**：

```
正常帧传输:
  站点 A: [前导码][====== 数据 (≥64字节) ======][FCS] ──▶ 成功
...
碰撞产生 Runt:
  站点 A: [前导码][== 部分数据 ==]
                         ...
                         [碰撞!] ←── 站点 B 同时发送
                              │
                              ▼
  结果: [前导码][== 残帧 (<64字节) ==] ──▶ 无效帧 (Runt)
```

**Late Collision（迟碰撞）**：

```
正常碰撞 (Early Collision):
  - 发生在发送前 64 字节期间
  - 发送方能检测到，会重传
  - Fragment-Free 可过滤
...
**延迟公式**：
```
T_fragment_free = T_64bytes + T_lookup
                = (64 × 8 / BW) + T_cam_lookup

示例:
  10 Mbps:   51.2 μs + 300ns ≈ 51.5 μs
  100 Mbps:  5.12 μs + 300ns ≈ 5.4 μs
  1 Gbps:    512 ns + 300ns  ≈ 812 ns
  100 Gbps:  5.12 ns + 300ns ≈ 305 ns
```
...
| 方面 | 说明 |
|------|------|
| ✅ 过滤碰撞帧 | 丢弃 Runt 帧（<64 字节） |
| ✅ 延迟适中 | 比 Store-and-Forward 低，比 Cut-Through 略高 |
| ✅ 兼容性好 | 适用于仍有碰撞可能的半双工环境 |
| ❌ 无完整 CRC | 无法检测 64 字节之后的错误 |
| ❌ 现代网络意义降低 | 全双工交换网络无碰撞，优势不明显 |

---

#### Adaptive Switching 模式（自适应转发）

**基本原理**：根据网络实时错误率动态切换转发模式，在性能和可靠性之间自动平衡。

**状态机模型**：

```
                         ┌─────────────────────────────────────────────┐
                         │           Adaptive Switching FSM            │
                         └─────────────────────────────────────────────┘
...
│              │                │                │                        │
│              ▼                ▼                ▼                        │
│      E < E_low         E_low ≤ E ≤ E_high    E > E_high                │
│         │                    │                  │                       │
│         ▼                    ▼                  ▼                       │
│   [切回Cut-Through]    [保持当前模式]    [切到Store-and-Fwd]            │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| 参数 | 典型值 | 说明 |
|------|--------|------|
| E_high (升级阈值) | 10⁻⁶ ~ 10⁻⁵ | 触发切换到 Store-and-Forward |
| E_low (降级阈值) | 10⁻⁷ ~ 10⁻⁶ | 触发切回 Cut-Through |
| T_window (监控窗口) | 1-10 秒 | 错误率统计周期 |
| T_recover (恢复延迟) | 30-60 秒 | 错误率低于阈值后等待时间 |
...
Switch
├── Port 1 (上行链路)
│   └── 模式: Store-and-Forward (固定)
│       原因: 连接不可信网络
│
├── Port 2-24 (服务器端口)
│   └── 模式: Adaptive
│       阈值: E_high=10⁻⁶, E_low=10⁻⁷
│       原因: 服务器网卡可靠性高
│
└── Port 25-48 (用户端口)
    └── 模式: Store-and-Forward (固定)
...
|------|------|
| ✅ 自动优化 | 根据网络状况自动选择最佳模式 |
| ✅ 平衡性能与可靠性 | 正常时低延迟，异常时高可靠 |
| ✅ 灵活配置 | 可按端口设置不同策略 |
| ❌ 复杂度高 | 需要维护状态机和监控逻辑 |
| ❌ 切换延迟 | 模式切换期间可能有短暂性能波动 |

### 3.2 转发模式对比总结

| 模式 | 转发前读取 | 典型延迟 (100GbE) | 错误检测 | 适用场景 |
|------|-----------|-------------------|----------|----------|
| **Fast-Forward** (Cut-Through) | 6 字节 | ~300-500ns | 无 | 网络核心层、低错误率环境 |
| **Fragment-Free** | 64 字节 | ~800ns | Runt/碰撞 | 折中方案、一般企业网 |
| **Store-and-Forward** | 完整帧 | 数μs (帧大小相关) | 完整 CRC | 接入层、需要 QoS、高错误率 |
| **Adaptive** | 动态 | 动态 | 动态 | 错误率波动的网络 |

### 3.3 Cut-Through 的限制条件
